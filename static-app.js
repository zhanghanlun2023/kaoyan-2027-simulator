...
const state = {page:"home", mode:"全日制", track:"普通硕士", university:"", subject:"思想政治理论", data:{}, paper:[], submitted:false};
...
function home(){
  ...
  <div class="metrics"><div class="metric"><span>覆盖统考科目</span><b>${state.data.syllabus.subjects.length} 门</b></div><div class="metric"><span>原创仿真题</span><b>${state.data.questions.length} 道</b></div><div class="metric"><span>MBA关联题量</span><b>${state.data.questions.filter(q=>["管理类综合能力","英语二"].includes(q.subject)).length} 道</b></div><div class="metric"><span>院校招生单位/校区</span><b>116 个</b></div></div>
...
function quiz(){
  ...
  if(!state.paper.length)state.paper=seededShuffle(pool,new Date().toISOString().slice(0,10)+state.subject).slice(0,Math.min(12,pool.length));
  return `${title("智能组卷","按真实科目代码和客观题分值训练；写作题提供自评提示，不冒充机器阅卷。")}${u?`<p class="muted">目标：${esc(u.name)} · ${state.mode} · ${state.track} · ${state.subject}</p>`:""}${state.track==="MBA 工商管理"?'<div class="notice info"><b>MBA 联考路径</b>：199管理类综合能力200分 + 204英语（二）100分。199写作须人工评阅，本系统不把关键词匹配伪装成正式分数。</div>':state.mode==="非全日制"?'<div class="notice info">在职训练建议：工作日做5—8题限时小测，周末再完成整卷。</div>':""}<p class="muted">当前科目题库：${pool.length} 道</p><button id="generate" class="secondary">换一套题</button><div id="question-list">${state.paper.map((q,i)=>questionHTML(q,i)).join("")}</div><button id="submit" class="primary">交卷并生成诊断</button><div id="score-box"></div>`;
}
function questionHTML(q,i){if(q.type==="essay")return `<article class="question" data-q="${q.id}"><div class="q-meta">第 ${i+1} 题 · ${esc(q.chapter)} · ${q.points}分 · ${esc(q.format_tag||"人工自评")}</div><h4>${esc(q.stem)}</h4><textarea class="essay-input" name="q_${q.id}" rows="9" placeholder="在这里列提纲或完成写作；提交后显示自评要点"></textarea><div class="answer" hidden></div></article>`;const multiple=q.type==="multiple",type=multiple?"checkbox":"radio";return `<article class="question" data-q="${q.id}"><div class="q-meta">第 ${i+1} 题 · ${esc(q.chapter)} · ${q.points}分 · ${esc(q.format_tag||"专项训练")} · 难度 ${"●".repeat(q.difficulty)}${"○".repeat(5-q.difficulty)}</div><h4>${esc(q.stem)}</h4>${Object.entries(q.options).map(([k,v])=>`<label class="option"><input type="${type}" name="q_${q.id}" value="${k}"><span><b>${k}.</b> ${esc(v)}</span></label>`).join("")}<div class="answer" hidden></div></article>`;}
function generatePaper(){const pool=state.data.questions.filter(q=>q.subject===state.subject);state.paper=seededShuffle(pool,Date.now().toString()).slice(0,Math.min(12,pool.length));render();}
function scorePaper(){let earned=0,total=0,correct=0,graded=0,essays=0;state.paper.forEach(q=>{const box=document.querySelector(`[data-q="${q.id}"] .answer`);box.hidden=false;if(q.type==="essay"){essays++;box.innerHTML=`<b>📝 自评要点</b><br>${esc(q.explanation)}<br><span class="muted">本题${q.points}分，需由教师或同伴依据结构、论证和语言人工评阅。</span>`;return;}total+=q.points;graded++;const picked=[...document.querySelectorAll(`input[name="q_${q.id}"]:checked`)].map(x=>x.value).sort();const expected=(Array.isArray(q.answer)?q.answer:[q.answer]).slice().sort();const ok=JSON.stringify(picked)===JSON.stringify(expected);if(ok){earned+=q.points;correct++;}box.parentElement.classList.add(ok?"correct":"incorrect");box.innerHTML=`<b>${ok?"✅ 回答正确":"❌ 需要复盘"}</b><br>正确答案：${expected.join("、")}<br>${esc(q.explanation)}`;});const accuracy=graded?correct/graded:0;$("#score-box").innerHTML=`<div class="result ${accuracy>=.8?"ok":"bad"}"><b>客观题得分 ${earned} / ${total}</b>　正确率 ${graded?Math.round(accuracy*100):0}%${essays?`<br>另有 ${essays} 道写作题待人工自评。`:""}<br><span class="muted">${accuracy>=.8?"客观题基础掌握较好，可以提高难度。":"建议逐题查看解析并回到对应章节复习。"}</span></div>`;$("#score-box").scrollIntoView({behavior:"smooth"});}
...
function about(){return `${title("项目说明","公开、可追溯、可继续扩充的考研训练底座。")}<div class="panel"><h3>数据原则</h3><ul><li>训练题均为原创仿真，不冒充历年真题。</li><li>客观题按对应科目的常见题型和单题分值标注；主观题不做虚假的机器自动评分。</li><li>五年真题模块保存索引和复盘方法，不传播来源不明的整套试卷。</li><li>MBA是专业学位路径，不等同于非全日制；本系统支持常见的在职非全日制MBA训练配置。</li><li>985、211是历史工程标签，现行信息同时核对“双一流”和学校招生章程。</li></ul><h3>当前边界</h3><p class="muted">当前覆盖${state.data.syllabus.subjects.length}门统考科目、${state.data.questions.length}道原创参数化专项题、116个原985/211招生单位或校区。院校自命题专业课及主观题人工批改仍需外部教师完成。</p><a class="link-btn" target="_blank" rel="noopener" href="https://github.com/zhanghanlun2023/kaoyan-2027-simulator">查看 GitHub 源码</a></div>`;}
