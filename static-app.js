...
function questionListHTML(){let previous="";return state.paper.map((q,i)=>{let heading="";if(q.set_id&&q.set_id!==previous){previous=q.set_id;const passage=q.passage_id?state.data.english2.passages[q.passage_id]:"";heading=`<section class="set-heading"><span class="pill">${esc(q.section)}</span>${passage?`<div class="passage">${esc(passage)}</div>`:""}</section>`;}return heading+questionHTML(q,i);}).join("");}
function quiz(){
  const isEnglish2=state.subject==="英语二",pool=isEnglish2?state.data.english2.questions:state.data.questions.filter(q=>q.subject===state.subject),u=university();
  if(!state.paper.length)state.paper=isEnglish2?buildEnglish2Paper(new Date().toISOString().slice(0,10)):seededShuffle(pool,new Date().toISOString().slice(0,10)+state.subject).slice(0,Math.min(12,pool.length));
  return `${title("智能组卷",isEnglish2?"英语二严格按完形、阅读A、阅读B、翻译、应用文和大作文组成100分完整仿真卷。":"按真实科目代码和客观题分值训练；写作题提供自评提示，不冒充机器阅卷。")}${u?`<p class="muted">目标：${esc(u.name)} · ${state.mode} · ${state.track} · ${state.subject}</p>`:""}${isEnglish2?'<div class="notice info"><b>204英语（二）完整结构</b>：完形20题10分；阅读A 20题40分；阅读B 5题10分；翻译15分；应用文10分；图表/情境作文15分。</div>':state.track==="MBA 工商管理"?'<div class="notice info"><b>MBA 联考路径</b>：199管理类综合能力200分 + 204英语（二）100分。199写作须人工评阅，本系统不把关键词匹配伪装成正式分数。</div>':state.mode==="非全日制"?'<div class="notice info">在职训练建议：工作日做5—8题限时小测，周末再完成整卷。</div>':""}<p class="muted">当前科目题库：${pool.length} 道${isEnglish2?" · 每次生成48题完整卷":""}</p><button id="generate" class="secondary">${isEnglish2?"换一套完整卷":"换一套题"}</button><div id="question-list">${questionListHTML()}</div><button id="submit" class="primary">交卷并生成诊断</button><div id="score-box"></div>`;
}
...
function generatePaper(){const seed=Date.now().toString();if(state.subject==="英语二")state.paper=buildEnglish2Paper(seed);else{const pool=state.data.questions.filter(q=>q.subject===state.subject);state.paper=seededShuffle(pool,seed).slice(0,Math.min(12,pool.length));}render();}
