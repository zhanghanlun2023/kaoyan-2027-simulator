...
function buildManagementPaper(seed){const bank=state.data.questions.filter(q=>q.subject==="管理类综合能力"),paperIds=[...new Set(bank.map(q=>q.paper_id).filter(Boolean))].sort();if(paperIds.length){const chosen=seededShuffle(paperIds,seed)[0];return bank.filter(q=>q.paper_id===chosen).sort((a,b)=>a.paper_order-b.paper_order);}const plan=[["数学基础·问题求解",15],["数学基础·条件充分性判断",10],["逻辑推理",30],["写作·论证有效性分析",1],["写作·论说文",1]],paper=[];for(const [chapter,count] of plan)paper.push(...seededShuffle(bank.filter(q=>q.chapter===chapter),seed+chapter).slice(0,count));return paper;}
...
function managementQuestionListHTML(){let previousChapter="",previousGroup="";return state.paper.map((q,i)=>{let heading="";if(q.chapter!==previousChapter){previousChapter=q.chapter;previousGroup="";heading+=`<section class="set-heading"><span class="pill">${esc(q.chapter)}</span></section>`;}if(q.group_id&&q.group_id!==previousGroup){previousGroup=q.group_id;heading+=`<section class="management-material"><div class="passage-toolbar"><span class="pill">公共材料题组</span><span>本材料对应后续3题</span></div><div>${esc(q.shared_stem||"")}</div></section>`;}return heading+questionHTML(q,i);}).join("");}
function questionListHTML(){if(state.subject==="英语二")return englishQuestionListHTML();if(state.subject==="管理类综合能力")return managementQuestionListHTML();return state.paper.map((q,i)=>questionHTML(q,i)).join("");}
function quiz(){
