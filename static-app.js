...
const state = {page:"home", mode:"全日制", university:"", subject:"思想政治理论", data:{}, paper:[], submitted:false};
const $ = (q, root=document) => root.querySelector(q);
const esc = value => String(value ?? "").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
const tag = u => u?.is_985 ? "原985 / 原211" : "原211";

async function boot(){
  const [questions,syllabus,papers,universities] = await Promise.all([
    "data/questions.json","data/syllabus.json","data/past_papers.json","data/universities.json"
  ].map(url=>fetch(url).then(r=>{if(!r.ok) throw new Error(url); return r.json()})));
  state.data={questions,syllabus,papers,universities};
  restore(); setupChrome(); route();
  $("#loading").hidden=true; $("#app").hidden=false;
}
function restore(){
  try{const saved=JSON.parse(localStorage.getItem("yantu-pref")||"{}"); Object.assign(state,{mode:saved.mode||state.mode,university:saved.university||"",subject:saved.subject||state.subject});}catch{}
}
function save(){localStorage.setItem("yantu-pref",JSON.stringify({mode:state.mode,university:state.university,subject:state.subject}));}
function setupChrome(){
  ...
  const subjects=state.data.syllabus.subjects.map(s=>s.name);
  $("#subject-select").innerHTML=subjects.map(s=>`<option>${esc(s)}</option>`).join("");
...
function chrome(){
  ...
  $("#university-select").value=state.university; $("#subject-select").value=state.subject;
}
...
function home(){
  const u=university(), tip=state.mode==="全日制"?"整段复习优先：按正式考试时长训练":"在职节奏：工作日小测 + 周末整卷";
  ...
  <div class="metrics"><div class="metric"><span>覆盖统考科目</span><b>7 门</b></div><div class="metric"><span>原创仿真题</span><b>${state.data.questions.length} 道</b></div><div class="metric"><span>院校招生单位/校区</span><b>116 个</b></div><div class="metric"><span>考纲状态</span><b>待官方发布</b></div></div>
  <div class="notice"><b>${state.mode} · ${esc(u?.name||"尚未确定目标院校")}</b><br><span class="muted">${tip}。全国统考科目使用同一题库；院校差异主要在专业目录、自命题、复试和培养安排。</span></div>
...
function universities(){
  ...
  <div class="notice info">${state.mode==="非全日制"?"非全日制常被口语称为“在职研究生”。全日制和非全日制执行相同考试招生政策和标准；原则上非全日制硕士招收在职定向就业人员。":"全日制通常为全脱产在校学习。考试科目、学制、学费、住宿和培养校区仍需逐校逐专业核对。"}</div>
...
function quiz(){
  const pool=state.data.questions.filter(q=>q.subject===state.subject),u=university();
  if(!state.paper.length)state.paper=seededShuffle(pool,new Date().toISOString().slice(0,10)+state.subject).slice(0,Math.min(8,pool.length));
  return `${title("智能组卷","全国统考题不因目标院校改变；院校选择用于报考路径和自命题核验。")}${u?`<p class="muted">目标：${esc(u.name)} · ${state.mode} · ${state.subject}</p>`:""}${state.mode==="非全日制"?'<div class="notice info">在职训练建议：工作日做5—8题限时小测，周末再完成整卷。</div>':""}<button id="generate" class="secondary">换一套题</button><div id="question-list">${state.paper.map((q,i)=>questionHTML(q,i)).join("")}</div><button id="submit" class="primary">交卷并生成诊断</button><div id="score-box"></div>`;
}
function questionHTML(q,i){const multiple=q.type==="multiple",type=multiple?"checkbox":"radio";return `<article class="question" data-q="${q.id}"><div class="q-meta">第 ${i+1} 题 · ${esc(q.chapter)} · ${q.points}分 · 难度 ${"●".repeat(q.difficulty)}${"○".repeat(5-q.difficulty)}</div><h4>${esc(q.stem)}</h4>${Object.entries(q.options).map(([k,v])=>`<label class="option"><input type="${type}" name="q_${q.id}" value="${k}"><span><b>${k}.</b> ${esc(v)}</span></label>`).join("")}<div class="answer" hidden></div></article>`;}
function generatePaper(){const pool=state.data.questions.filter(q=>q.subject===state.subject);state.paper=seededShuffle(pool,Date.now().toString()).slice(0,Math.min(8,pool.length));render();}
function scorePaper(){let earned=0,total=0,correct=0;state.paper.forEach(q=>{total+=q.points;const picked=[...document.querySelectorAll(`input[name="q_${q.id}"]:checked`)].map(x=>x.value).sort();const expected=(Array.isArray(q.answer)?q.answer:[q.answer]).slice().sort();const ok=JSON.stringify(picked)===JSON.stringify(expected);if(ok){earned+=q.points;correct++;}const box=document.querySelector(`[data-q="${q.id}"] .answer`);box.hidden=false;box.parentElement.classList.add(ok?"correct":"incorrect");box.innerHTML=`<b>${ok?"✅ 回答正确":"❌ 需要复盘"}</b><br>正确答案：${expected.join("、")}<br>${esc(q.explanation)}`;});$("#score-box").innerHTML=`<div class="result ${correct/state.paper.length>=.8?"ok":"bad"}"><b>得分 ${earned} / ${total}</b>　正确率 ${Math.round(correct/state.paper.length*100)}%<br><span class="muted">${correct/state.paper.length>=.8?"基础掌握较好，可以提高难度。":"建议逐题查看解析并回到对应章节复习。"}</span></div>`;$("#score-box").scrollIntoView({behavior:"smooth"});}
...
function about(){return `${title("项目说明","公开、可追溯、可继续扩充的考研训练底座。")}<div class="panel"><h3>数据原则</h3><ul><li>训练题均为原创仿真，不冒充历年真题。</li><li>五年真题模块保存索引和复盘方法，不传播来源不明的整套试卷。</li><li>985、211是历史工程标签，现行信息同时核对“双一流”和学校招生章程。</li><li>全日制和非全日制执行相同考试招生政策和标准。</li></ul><h3>当前边界</h3><p class="muted">当前覆盖7门通用统考科目、116个原985/211招生单位或校区。院校自命题专业课、政治主观题、英语作文自动批改和完整大题仍需继续扩充。</p><a class="link-btn" target="_blank" rel="noopener" href="https://github.com/zhanghanlun2023/kaoyan-2027-simulator">查看 GitHub 源码</a></div>`;}
