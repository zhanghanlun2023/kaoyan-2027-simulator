...
function questionListHTML(){let previous="";return state.paper.map((q,i)=>{let heading="",group=q.set_id||(state.subject==="管理类综合能力"?q.chapter:"");if(group&&group!==previous){previous=group;const passage=q.passage_id?state.data.english2.passages[q.passage_id]:"";heading=`<section class="set-heading"><span class="pill">${esc(q.section||q.chapter)}</span>${passage?`<div class="passage">${esc(passage)}</div>`:""}</section>`;}return heading+questionHTML(q,i);}).join("");}
function quiz(){
